...
/**
 * common superclass for Bindable and Managed second pass evaluators.
 */
public abstract class GenerativeSecondPassEvaluator extends EvaluatorAdapter
{
	...
	{
		Map renamedVariableMap = (Map) unit.getContext().getAttribute(flex2.compiler.Context.RENAMED_VARIABLE_MAP);
		if (renamedVariableMap == null)
		{
			renamedVariableMap = new HashMap();
			unit.getContext().setAttribute(flex2.compiler.Context.RENAMED_VARIABLE_MAP, renamedVariableMap);
		}
	...
	protected ProgramNode generateSupportCode(Context context, String className)
	{
		...
		try
		{
			template = VelocityManager.getTemplate(getTemplatePath() + templateName + ".vm");
		}
		catch(Exception e) {}
		...
		if (template != null)
		{
			try
			{
				...
				context.setHandler(new flex2.compiler.as3.Compiler.CompilerHandler());
				context.statics.handler = context.getHandler();

				Parser parser = new Parser(context, stringWriter.toString(), generatedName);
	 ...
	 */
	private void prepMetaDataNode(Context context, MetaDataNode metaData)
	{
