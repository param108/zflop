        ...
        implements TagValues
{
    ...
    public void finish()
    {
        ...
        if (debug != null)
        {
            ...
            byte[] md5 = MD5.getDigest(writer.getByteArray(), writer.size());
    ...
    private void encodeLineStyle(LineStyle lineStyle, SwfEncoder w, int shape)
    {
        ...
        if (shape == stagDefineShape6 && lineStyle.hasFillStyle())
        {
            encodeFillStyle( lineStyle.fillStyle, w, shape );
        }
        else if ((shape == stagDefineShape3) || (shape == stagDefineShape6))
        {
    ...
    private void encodeFillStyle(FillStyle style, SwfEncoder w, int shape)
    {
        ...
        case FillStyle.FILL_SOLID: // 0x00
            if ((shape == stagDefineShape3) || (shape == stagDefineShape6)) encodeRGBA(style.color, w);
    ...
    private void encodeGradRecord(GradRecord record, SwfEncoder w, int shape)
    {
        ...
        if ((shape == stagDefineShape3) || (shape == stagDefineShape6))
    ...
    private void encodeShapeWithStyle(ShapeWithStyle shapeWithStyle, SwfEncoder w, int shape)
    {
        ...
        encodeShape(shapeWithStyle, w, shape, shapeWithStyle.fillstyles.size(), shapeWithStyle.linestyles.size());
    }
    ...
    private void encodeDefineText(DefineText tag, SwfEncoder w, int type)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            TextRecord record = (TextRecord)tag.records.get(i);
    ...
    private void encodeFilterList(List filters, SwfEncoder w)
    {
