...
public class FileInfoCache implements Comparator
{
	...
	void loadCache()
	{
		...
		// trim the file list
		ArrayList fa = trimFileList(files);
		m_files = (SourceFile[]) fa.toArray( new SourceFile[fa.size()] );
	...
	boolean loadSwfFiles(ArrayList ar, SwfInfo swf)
	{
		boolean worked = true;
		try
		{
	 ...
	 */
	ArrayList trimFileList(ArrayList files)
	{
		...
		for(int i=0; i<size; i++)
		{
			...
			SourceFile fi = (SourceFile)files.get(i);
    ...
    public SourceFile[] getFiles(String matchString)
    {
		...
		SourceFile[] fileArray = (SourceFile[]) fileList.toArray( new SourceFile[fileList.size()] );
