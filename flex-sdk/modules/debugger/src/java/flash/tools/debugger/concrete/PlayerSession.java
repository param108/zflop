...
public class PlayerSession implements Session, DProtocolNotifierIF, Runnable
{
	...
	private volatile boolean m_isHalted; // WARNING -- accessed from multiple threds
	...
	private String executeAppleScript(String[] appleScript, String[] argv)
	{
		...
		try
		{
			...
			Process osascript = Runtime.getRuntime().exec((String[]) execArgs.toArray(new String[execArgs.size()]));
	...
	private String executeAppleScript(String appleScriptFilename, String[] argv) throws IOException
	{
		...
		try {
			...
			String line;
			List appleScriptLines = new ArrayList();
			while ( (line=reader.readLine()) != null )
				appleScriptLines.add(line);
			String[] lines = (String[]) appleScriptLines.toArray(new String[appleScriptLines.size()]);
	...
	public void terminate()
	{
		...
		try
		{
			...
			// terminate.
			boolean requestTerminate = (m_process != null);
		...
		if (!playerWillTerminateItself)
		{
			if (System.getProperty("os.name").toLowerCase().startsWith("mac os x")) //$NON-NLS-1$ //$NON-NLS-2$
			{
				...
				new Thread(r, "Terminate Mac debug target").start(); //$NON-NLS-1$
			}
	
			// if we have a process pop it
			if (m_process != null)
			{
				try { m_process.destroy(); } catch(Exception e) {}
			}
		}
	...
	public Watch setWatch(Value v, String memberName, int kind) throws NoResponseException, NotConnectedException
	{
		...
		if (addWatch(v.getId(), memberName, kind, tag))
		{
	...
	private void pullUpActivationObjectVariables(int depth) throws NotSuspendedException, NoResponseException, NotConnectedException
	{
		...
		Map varmap = new LinkedHashMap(frameVars.length); // preserves order
		...
		while (iter.hasNext())
		{
			...
			{
				if (next.getName().equals("this")) //$NON-NLS-1$
	 ...
	 */
	public Value getRawValue(int valueId)
	{
	 ...
	 * a value for that id.
	 */
	public Value getPreviousValue(int valueId)
	{
...
//		System.out.println("setmsg id="+id+",name="+name+",t="+type+",value="+value);
	...
	public void waitForEvent() throws NotConnectedException, InterruptedException
	{
		...
		{
			synchronized (eventNotifier) { eventNotifier.wait(); }
		}
	...
	java.io.Writer traceFile() throws IOException
	{
		if (m_trace == null)
		{
			...
			try
			{
				...
				// property dump
				java.util.Iterator keys = m_prefs.keySet().iterator();
				while(keys.hasNext())
				{
