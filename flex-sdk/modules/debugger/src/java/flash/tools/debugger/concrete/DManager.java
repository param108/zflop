...
{
	private final HashMap		m_parms;
	private final IntMap		m_source;	   /* WARNING: accessed from multiple threads */
	private final ArrayList		m_breakpoints; /* WARNING: accessed from multiple threads */
	private final ArrayList		m_swfInfo;     /* WARNING: accessed from multiple threads */
	private final ArrayList		m_watchpoints; /* WARNING: accessed from multiple threads */
	 ...
	 */
	private ArrayList		m_previousFrames;
	...
	private LinkedList		m_event;		/* our event queue; WARNING: accessed from multiple threads */
	...
	private boolean		m_inGetterSetter;
	private FaultEvent	m_faultEventDuringGetterSetter;

	private Map			m_options = new HashMap();	// Player options that have been queried by OutGetOption, and come back via InOption
	...
	public DSwfInfo[] getSwfInfos()
	{
		...
		{
			return (DSwfInfo[])m_swfInfo.toArray( new DSwfInfo[m_swfInfo.size()] );
		}
	}
	...
	public DSwfInfo getSwfInfo(int at)
	{
		synchronized (m_swfInfo)
		{
			return (DSwfInfo)m_swfInfo.get(at);
		}
	}
...
//			System.out.println("attaching scripts "+min+"-"+max+" to "+info.getUrl());
	...
	public DModule getSource(int id)
	{
		synchronized (m_source)
		{
			return (DModule)m_source.get(id);
		}
	}
	...
	public DModule[] getSources()
	{
		synchronized (m_source)
		{
			...
			count = 0;
			Iterator itr = m_source.iterator();
			while(itr.hasNext())
			{
	 			Map.Entry entry = (Map.Entry)itr.next();
				DModule sf = (DModule) entry.getValue();
				ar[count++] = sf;
			}
			return ar;
		}
	...
	{
		DValue v = (DValue)m_values.get((int)id);
		return v;
	}
	...
	{
		return (DValue) m_previousValues.get((int)id);
	}
	...
	public DLocation getBreakpoint(int id)
	{
		synchronized (m_breakpoints)
		{
			...
			if (which > -1)
				loc = (DLocation)m_breakpoints.get(which);
			return loc;
		}
	}
	...
	int findBreakpoint(int id)
	{
		synchronized (m_breakpoints)
		{
			...
			{
				DLocation l = (DLocation) m_breakpoints.get(i);
				if (l.getId() == id)
					which = i;
			}
			return which;
	...
	public DLocation[] getBreakpoints()
	{
		synchronized (m_breakpoints)
		{
			return (DLocation[]) m_breakpoints.toArray(new DLocation[m_breakpoints.size()]);
		}
	}
	 ...
	 * Watchpoints
	 */
	public DWatch		getWatchpoint(int at)	{ synchronized (m_watchpoints) { return (DWatch)m_watchpoints.get(at); } }
	public int			getWatchpointCount()	{ synchronized (m_watchpoints) { return m_watchpoints.size(); } }
	public DWatch[]		getWatchpoints()		{ synchronized (m_watchpoints) { return (DWatch[])m_watchpoints.toArray( new DWatch[m_watchpoints.size()] ); } }
	...
	DWatch removeWatchpoint(int tag)
	{
		synchronized (m_watchpoints)
		{
			...
			if (at > -1)
				w = (DWatch)m_watchpoints.remove(at);
			return w;
		}
	}
	...
	public DStackContext		getFrame(int at)		{ return (DStackContext)m_frames.get(at);}
	public int					getFrameCount()			{ return m_frames.size();}
	public DStackContext[]		getFrames()				{ return (DStackContext[])m_frames.toArray( new DStackContext[m_frames.size()] );	}
	...
	private void mapOldFramesToNew() {
		...
		{
			DStackContext oldFrame = (DStackContext) m_previousFrames.get(oldDepth);
			DStackContext newFrame = (DStackContext) m_frames.get(newDepth);
	...
	public DebugEvent nextEvent()
	{
		...
		{
			if (m_event.size() > 0)
				s = (DebugEvent) m_event.removeFirst();
		}
		return s;
	}
	...
	/**
	 * Attach this manager to listen to the socket.
	 */
	public void attach(DProtocol d)
	{
		d.addListener(this);
	}

	public void release(DProtocol d)
	{
...
//					System.out.println("   this="+id+",@"+(bpi&0xffff)+":"+(bpi>>16)+",stack="+stack);
			...
			case DMessage.InGetVariable:
			{
				...
				if (parent != null && parent.getClassHierarchy(true) == null) {
					parent.setClassHierarchy((String[]) classes.toArray(new String[classes.size()]), highestLevelWithMembers+1);
				}
				...
				for(int i=0; i<count; i++)
				{
					...
					if (id != 0)
					{
						...
						for(int j=0; j<num; j++)
						{
							...
							local2global.put((int)local, new Integer((int)global));
...
//					System.out.println("[SWFLOAD] Loaded "+path+", size="+swfSize+", scripts="+scriptCount);
	 ...
	 * locals and _global.
	 */
	void populateRootNode(DValue frame, ArrayList orderedChildList)
	{
	 ...
	 */
	void populateFrame(int depth, ArrayList frameVars)
	{
		...
		Iterator e = frameVars.iterator();
		while(e.hasNext())
		{
			DVariable v = (DVariable)e.next();
			...
			else if (name.equals(ARGUMENTS_MARKER))
			{
				...
				try { nArgs = (int) ((Number)(v.getValue().getValueAsObject())).doubleValue(); } catch(NumberFormatException nfe){}
			}
...
//				System.out.println("<unknown>");
	...
	public static String convertToURI(String playerURL)
	{
		...
		StringBuffer sb = new StringBuffer(playerURL);
		while(index > 0)
		{
			sb.setCharAt(index, ':');
	...
	/**
	 * Tell us that a getter or setter is about to be executed.  If a
	...
	/**
	 * Informs us that a getter or setter is no longer executing, and
	 * returns the fault, if any, which occurred while the getter/
	 * setter was executing.
	 */
	public FaultEvent endGetterSetter()
	{
	...
	private void handleFaultEvent(FaultEvent faultEvent)
	{
		if (m_inGetterSetter)
		{
			if (m_faultEventDuringGetterSetter == null) // only save the first fault
			{
	...
	{
		return (String) m_options.get(optionName);
	}
}
